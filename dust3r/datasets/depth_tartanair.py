...
class TartanAirDepth(BaseMultiViewDataset):
    ...
    def __init__(self, *args, ROOT: str, **kwargs):
        self.ROOT = ROOT
        self.dataset_label = "TartanAirDepth"
        ...
        for dir in sorted(os.listdir(split_dir)):
            for subdir in sorted(os.listdir(os.path.join(self.ROOT, dir, 'Hard'))):
                list_data.append(f"{self.ROOT}/{dir}/Hard/{subdir}")
        seq_cnt = 0

        for seq in list_data:
            seq_cnt += 1
            frame_files  = [f for f in sorted(os.listdir(seq)) if f.lower().endswith((".png"))]
            depth_files  = [f for f in sorted(os.listdir(seq)) if f.lower().endswith((".npy"))]
            camera_files = [f for f in sorted(os.listdir(seq)) if f.lower().endswith((".npz"))]
    ...
    def __getitem__(self, index: Any):
        ...
        for i in range(self.num_views):
            ...
            extrinsic = camera["camera_pose"]
            intrinsic = camera["camera_intrinsics"]
