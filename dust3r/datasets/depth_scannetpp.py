...
class ScanNetPPDepth(BaseMultiViewDataset):
    ...
    def __init__(self, *args, ROOT: str, **kwargs):
        self.ROOT = ROOT
        self.dataset_label = "ScanNetPPDepth"
        ...
        for dir in sorted(os.listdir(split_dir)):
            if 'all_metadata.npz' not in dir:
        ...
        for seq in list_data:
            seq_cnt += 1
            frame_files  = [f for f in sorted(os.listdir(f"{seq}/images/")) if f.lower().endswith((".jpg"))]
            depth_files  = [f for f in sorted(os.listdir(f"{seq}/depth/")) if f.lower().endswith((".png"))]
            ...
            self.images.extend([osp.join(seq, 'images', ff) for ff in frame_files])
            self.depths.extend([osp.join(seq, 'depth', ff) for ff in depth_files])
