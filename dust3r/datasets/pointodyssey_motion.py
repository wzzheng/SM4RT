...
def _resize_center_crop_mask(img: np.ndarray, target_hw: Tuple[int, int]) -> Tuple[np.ndarray, float, int, int]:
    th, tw = target_hw  # (H, W)
    H, W = img.shape[:2]
    if th <= 0 or tw <= 0:
        return img, 1.0, 0, 0
...
class PointOdysseyDepth(BaseMultiViewDataset):
    ...
    def __init__(self, *args, ROOT: str, **kwargs):
        ...
        list_data = [dir for dir in sorted(os.listdir(split_dir)) if ('mp4' not in dir) and ('py' not in dir)]
        seq_cnt = 0
    ...
    def __getitem__(self, index: Any):
        ...
        # print(self.images[0])
        # np.load(osp.join(self.ROOT, getattr(self, "split", "train"), self.images[0]"), mmap_mode="r")
        scene_name_, basename_ = img_list_selected[0].split(os.sep, 1)
        img = Image.open(osp.join(self.ROOT, getattr(self, "split", "train"), scene_name_, "rgbs", osp.basename(basename_)))
        width, height = img.size

        anno_ = np.load(osp.join(self.ROOT, getattr(self, "split", "train"), self.scenes[scene_id], "anno.npz"), mmap_mode="r")
        trajs_3d  = anno_["trajs_3d"][img_idxs_local]
        extrinsic = anno_['extrinsics'][img_idxs_local]
        intrinsic = anno_['intrinsics'][img_idxs_local]
        visibs    = anno_["visibs"][img_idxs_local]
        ...
        scene_scale = _scene_scale_from_track3d(torch.tensor(tracks3d_world).unsqueeze(0)).numpy()
        tracks3d_world /= scene_scale
        tracks3d_cam /= scene_scale
        tracks3d_reproj = project_3d2d(torch.tensor(tracks3d_world).unsqueeze(0), torch.tensor(intrinsic))[0].numpy()
        tracks3d_reproj_uv, _ = tracks3d_reproj[...,:2], tracks3d_reproj[...,2]
        # hstart = int((H - 294)/2)
        mask = (tracks3d_reproj_uv[0][..., 0] < width) & (tracks3d_reproj_uv[0][..., 0] > 0) & (tracks3d_reproj_uv[0][..., 1] < height) & (tracks3d_reproj_uv[0][..., 1] > 0) & (visibs[0] == True)
        # tracks3d_reproj_uv[..., 1] -= hstart
        visibs = visibs[:,mask]
        # print(visibs.shape)
        # exit()
        ...
        for i in range(self.num_views):
            ...
            img_path = osp.join(self.ROOT, getattr(self, "split", "train"), scene_name, "rgbs", osp.basename(basename))
            mask_path = osp.join(self.ROOT, getattr(self, "split", "train"), scene_name, "masks", osp.basename('mask_'+basename[4:9]+'.png'))

            # mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
            # print(mask.shape)
            # mask_H, mask_W = mask.shape[:2]
            # mask = cv2.resize(mask, (H, W), interpolation=cv2.INTER_NEAREST_EXACT)
            # mask, _, _, _ = _resize_center_crop_mask(mask, (H, W))
            # background = (mask == 0)
            ...
            # img_out = img_aug.astype(np.float32) / 255.0
            # print(img_aug.shape)
            # exit()
            # print(torch.tensor(img_aug).max(), torch.tensor(img_aug).min())
            # print(ImgNorm(img_aug).max(), ImgNorm(img_aug).min())
            # exit()
