...
class ResizedDataset(EasyDataset):
    ...
    def set_epoch(self, epoch):
        # Guard against empty underlying dataset to avoid ZeroDivisionError
        if len(self.dataset) == 0:
            raise ValueError(
                f"ResizedDataset wraps an empty dataset (repr={repr(self.dataset)}). Check ROOT/split/filters; cannot resize from 0 elements."
            )
