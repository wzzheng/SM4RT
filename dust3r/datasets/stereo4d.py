...
class Stereo4D(BaseMultiViewDataset):
    ...
    def __init__(self, *args, ROOT: str, **kwargs):
        ...
        list_data = [dir for dir in sorted(os.listdir(split_dir)) if 'mp4' not in dir]
        seq_cnt = 0
    ...
    def __getitem__(self, index: Any):
        ...
        anno_path = osp.join(self.ROOT, self.scenes[scene_id], "annotations.npz")
        anno = np.load(anno_path, mmap_mode="r")

        trajs_3d = anno['trajs_3d'][img_idxs_local]
        visibilities = anno['visibilities'][img_idxs_local]
        intrinsic = anno['intrinsics'][img_idxs_local]
        extrinsic = anno['extrinsics'][img_idxs_local]
        ...
        mask = (tracks3d_reproj_uv[0][..., 0] < W) & (tracks3d_reproj_uv[0][..., 0] > 0) & (tracks3d_reproj_uv[0][..., 1] < H) & (tracks3d_reproj_uv[0][..., 1] > 0) & (visibilities[0].astype(bool))
        ...
        #     imgdir = osp.join(self.ROOT, scene_name, "rgbs", osp.basename(basename))
        ...
        # imageio.mimsave('./stereo_exp.gif', frames_track, fps=6)
        # frames = []
        # for i in range(24):
        #     scene_name, basename = img_list_selected[i].split(os.sep, 1)
        #     imgdir = osp.join(self.ROOT, scene_name, "rgbs", osp.basename(basename))
        #     img = cv2.imread(imgdir)
        #     H, W, _ = img.shape
        #     left = int((W-H) / 2)
        ...
        # imageio.mimsave('./stereo_vid.gif', frames, fps=6)
        # exit()
