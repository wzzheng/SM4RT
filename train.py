...
import dust3r.utils.misc as misc  # noqa
from dust3r.utils.misc import NativeScalerWithGradNormCount as NativeScaler  # noqa
...
def train(args):
    ...
    if args.pretrained and not args.resume:
        ...
        if 'model' in ckpt:
            ckpt = {k.replace('module.', ''): v for k, v in ckpt['model'].items() 
                # if 'twist_decoder' not in k
            }
        # twist_decoder
        # print(ckpt.keys())
        printer.info(model.load_state_dict(ckpt, strict=False))
    ...
    if args.freeze_module is not None:
        GLOB_FLAGS = (
        ...
        for name, mod in model.named_modules():
            # if 'aggregator_motion' in name:
            #     continue
            skip_keywords = ['twist_decoder', 'assign_head', 'motion_base_decoder']
            # skip_keywords = ['twist_decoder']
            # skip_keywords = []
            if any(keyword in name for keyword in skip_keywords):
                continue
            if any(fnmatch.fnmatch(name, p, flags=GLOB_FLAGS) for p in args.freeze_module):
                mod.eval()
                param_iter = mod.parameters()
                for p in param_iter:
...
def train_one_epoch(
    ...
    for data_iter_step, batch in enumerate(data_iter):
        ...
        with accelerator.accumulate(model):
            ...
            # for view in batch:
            #     if view["img"].shape[-1] == 3:
            #         view["img"] = (view["img"].permute(0, 3, 1, 2) + 1.0) / 2.0
            #     else:
