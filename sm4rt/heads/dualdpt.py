...
from sm4rt.utils.head_utils import Permute, create_uv_grid, custom_interpolate, position_grid_to_embed
...
class DualDPT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.scratch.output_conv1_aux = nn.ModuleList([self._make_aux_out1_block(head_features_1) for _ in range(self.aux_levels)])

        use_ln = True
        ln_seq = ([Permute((0, 2, 3, 1)), nn.LayerNorm(head_features_2), Permute((0, 3, 1, 2))] if use_ln else [])
        self.scratch.output_conv2_aux = nn.ModuleList([nn.Sequential(
            nn.Conv2d(head_features_1 // 2, head_features_2, kernel_size=3, stride=1, padding=1),
            *ln_seq,
            nn.ReLU(inplace=True),
    ...
    def forward(self, patch_token: torch.Tensor, H: int, W: int, patch_start_idx: int, chunk_size: int = 8,) -> Dict[str, torch.Tensor]:
        B, _, S, N, C = patch_token.shape
        feats = [patch_token[0,i] for i in range(patch_token.shape[1])]
        if chunk_size is None or chunk_size >= S:
            out_dict = self._forward_impl(feats, H, W, patch_start_idx)
            out_dict = self._reshape_output(out_dict, B, S)
            return Dict(out_dict)
        out_dicts = []
        for s0 in range(0, B * S, chunk_size):
            s1 = min(s0 + chunk_size, B * S)
            out_dict = self._forward_impl([feat[s0:s1] for feat in feats], H, W, patch_start_idx,)
            out_dicts.append(out_dict)
        out_dict = self._concat_outputs(out_dicts)
        out_dict = self._view_output(out_dict, B, S)
        return Dict(out_dict)
    
    def _reshape_output(self, out_dict: dict, B: int, S: int) -> dict:
        result = {}
        for k, v in out_dict.items():
            if isinstance(v, list):
                result[k] = [t.reshape(B, S, *t.shape[1:]) for t in v]
            else:
                result[k] = v.reshape(B, S, *v.shape[1:])
        return result
    
    def _concat_outputs(self, out_dicts: List[dict]) -> dict:
        result = {}
        for k in out_dicts[0].keys():
            if isinstance(out_dicts[0][k], list):
                num_levels = len(out_dicts[0][k])
                result[k] = [
                    torch.cat([od[k][i] for od in out_dicts], dim=0)
                    for i in range(num_levels)
                ]
            else:
                result[k] = torch.cat([od[k] for od in out_dicts], dim=0)
        return result
    
    def _view_output(self, out_dict: dict, B: int, S: int) -> dict:
        result = {}
        for k, v in out_dict.items():
            if isinstance(v, list):
                result[k] = [t.view(B, S, *t.shape[1:]) for t in v]
            else:
                result[k] = v.view(B, S, *v.shape[1:])
        return result

    def _forward_impl(self, feats: List[torch.Tensor], H: int, W: int, patch_start_idx: int,) -> Dict[str, torch.Tensor]:
        B, _, C = feats[0].shape
        ...
        aux_pred_list = []
        aux_conf_list = []
        for level_idx, aux_feat in enumerate(fused_aux_pyr):
            if self.pos_embed:
                aux_feat = self._add_pos_embed(aux_feat, W, H)
            aux_logits = self.scratch.output_conv2_aux[level_idx](aux_feat)
            fmap_aux = aux_logits.permute(0, 2, 3, 1)
            aux_pred_level = self._apply_activation_single(fmap_aux[..., :-1], "linear")
            aux_conf_level = self._apply_activation_single(fmap_aux[..., -1], self.conf_activation)
            aux_pred_list.append(aux_pred_level)
            aux_conf_list.append(aux_conf_level)

        aux_pred = aux_pred_list[-1]
        aux_conf = aux_conf_list[-1]
        return {
            self.head_main: main_pred.squeeze(-1),
            f"{self.head_main}_conf": main_conf,
            self.head_aux: aux_pred,
            f"{self.head_aux}_conf": aux_conf,
            f"{self.head_aux}_list": aux_pred_list,
            f"{self.head_aux}_conf_list": aux_conf_list,
        }
...
def _make_fusion_block(features: int, size: Tuple[int, int] = None, has_residual: bool = True, groups: int = 1, inplace: bool = False,) -> nn.Module:
...
class FeatureFusionBlock(nn.Module):
    def __init__(
        self, features: int, activation: nn.Module,
        deconv: bool = False, bn: bool = False, expand: bool = False, align_corners: bool = True, size: Tuple[int, int] = None, has_residual: bool = True,
        groups: int = 1,
    ) -> None:
        super().__init__()
